...
def offset_hull_mesh(hull) -> SurfaceMesh:
    """Mirror a public ``OffsetHull`` surface with outward body normals."""

    x = np.asarray(hull.x_m, dtype=float)
    z_axis = np.asarray(hull.z_m, dtype=float)
    ...
    for side_index, side in enumerate((-1.0, 1.0)):
        ...
        vertices.extend((x[i], side * breadth[i, j], z_axis[j]) for i in range(nx) for j in range(nz))
        for i in range(nx - 1):
            for j in range(nz - 1):
                a = base + i * nz + j
                b, c, d = a + nz, a + nz + 1, a + 1
                faces.extend(((a, b, c), (a, c, d)) if side < 0.0 else ((a, c, b), (a, d, c)))
    vertices_array = np.asarray(vertices, dtype=float)
    faces_array = np.asarray(faces, dtype=int)
    tolerance = max(float(hull.metadata.length_ref_m), 1.0) * 1.0e-12
...
def rectangular_free_surface(hull, nx=17, ny=13, upstream=1.0, downstream=2.0, lateral=1.0) -> SurfaceMesh:
    ...
    rows = []
    for xv in xs:
        half_breadth = float(np.interp(xv, hull.x_m, hull.half_breadth_m[:, 0], left=0.0, right=0.0))
