...
Figure~\ref{fig:byhull} puts the computed and measured curves side by side, one panel per
hull and on common axes, which is the most direct form of the comparison.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../figures/resistance_by_hull.pdf}
\caption{Computed wave resistance and measured residuary resistance versus $\Fn$ for
(a) Sysser 01 on a 280-panel mesh and (b) Sysser 50 on a 320-panel mesh, both at the static
attitude and both by the pressure route. Residuary resistance is a proxy for wave resistance
rather than a measurement of it: it follows from the measured total resistance by subtracting
an ITTC-57 friction line at form factor zero, so it also contains nonlinear and viscous-form
contributions.}
\label{fig:byhull}
\end{figure}
